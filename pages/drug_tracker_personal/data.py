...
from pages.drug_tracker.constants import ALCOHOL_DICT, DATABASE_ID, WEED_DICT
...
def calculate_weed_density(row):
    if row:
        if row[0] == ">":
            count = 0.5
            row = row[4:].strip()
            ...
            count = int(row[0])
            row = row[1:].strip()
        drug = row.split(" mit ")[0]
        people_count = int("".join([i for i in row if i.isdigit()]).strip())
        return count * WEED_DICT[drug] / people_count
...
def initial_clean(df):
    ...
    df["Weed"] = df["properties~Weed~multi_select"].apply(lambda row: get_names(row))
    df.drop("properties~Weed~multi_select", axis=1, inplace=True)
    df["Weed-Intake"] = df["Weed"].apply(lambda row: calculate_weed_density(row))
